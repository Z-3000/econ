...
## [2025-12-09] 경제지표 수집 개선

### 배경
- 한국은행 ECOS API의 일부 지표(기준금리, 콜금리, 두바이유, 금 시세)가 당일 데이터 미발표로 수집 실패
- Telegram 알림에서 실패 원인 파악 어려움

### 변경 내용

#### 1. ECOS API 조회 기간 확장
- **이전**: 당일(`today`)만 조회 → 데이터 미발표 시 실패
- **변경**: 최근 7일/3개월 기간 조회 후 최신 데이터 사용
- **효과**: 실패율 대폭 감소 (3/7 → 7/7 성공)

```python
# 변경 전
indicators = [(f"722Y001/D/{today}/{today}/0101000", "콜금리")]

# 변경 후
seven_days_ago = (today - pd.Timedelta(days=7)).strftime('%Y%m%d')
indicators = [(f"722Y001/D/{seven_days_ago}/{today_str}/0101000", "콜금리", "D")]
```

#### 2. 원자재 데이터 소스 변경
- **이전**: ECOS API (두바이유, 금 시세) → 데이터 미제공으로 항상 실패
- **변경**: yfinance로 대체
  - WTI 유가 (`CL=F`)
  - 금 선물 (`GC=F`)
...
#### 3. Telegram 알림 개선
- **추가**: "📅 과거 데이터 사용" 섹션
- 당일 데이터가 아닌 경우 날짜 표시 (예: `기준금리(11월)`, `콜금리(12/06)`)
- 사용자가 데이터 시점을 명확히 파악 가능
...
| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| 경제지표 성공률 | 3/7 (43%) | 7/7 (100%) |
| 원/달러 환율 | ✅ | ✅ |
| 원/엔 환율 | ✅ | ✅ |
| 원/유로 환율 | ✅ | ✅ |
| 기준금리 | ❌ 실패 | ✅ (과거 데이터) |
| 콜금리 | ❌ 실패 | ✅ (과거 데이터) |
| 두바이유/WTI | ❌ 실패 | ✅ yfinance |
| 금 시세 | ❌ 실패 | ✅ yfinance |
...
- **데이터 저장**: `/WD4T/econ/00_data_raw/archive/gdp_2010_2025.csv`

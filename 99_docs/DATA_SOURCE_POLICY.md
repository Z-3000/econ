...
#### 데이터 수집 개선 (2025-12-09)
- **기간 확장 조회**: 일간 지표는 최근 7일, 월간 지표는 최근 3개월 기간 조회
- **최신 데이터 사용**: 조회 결과 중 가장 최근 값 사용 (공휴일/주말 대응)
- **과거 데이터 추적**: Telegram 알림에 과거 날짜 데이터 사용 시 표시
...
#### 선정 이유 (2025-12-09 변경)
- **ECOS API 한계**: 두바이유/금 시세가 ECOS 902Y007 통계에서 조회 불가
- **대안 선택**: yfinance에서 원자재 선물 데이터로 대체

#### 수집 지표
| 지표 | 티커 | 설명 |
|------|------|------|
| WTI 유가 | CL=F | 서부텍사스중질유 선물 (국제유가 대표) |
| 금 선물 | GC=F | COMEX 금 선물 |

#### 장점
- 실시간성: 장중 가격 조회 가능
- 안정성: yfinance 통합 사용으로 주가와 동일 수집 로직
- 무료: 추가 API 키 불필요

### 3.3 미국 경제지표: FRED API
...
### 5.1 Cron 스케줄 설계 원칙
- **마감 후 단일 수집**: 장마감+버퍼(10분) 이후 1회만 실행해 확정 일봉만 적재
- **중복 방지**: `bar_date + ticker` 키로 덮어쓰기 (append 금지)
- **지연 탐지**: 게시 지연 시 `status=stale`로 표기하고 알림/대시보드에서 표시

### 5.2 수집 시간대 설정
| 시간 | 수집 항목 | 근거 |
|------|-----------|------|
| 06:10 | 미국 주가 + 경제지표 + 뉴스 | 미국 장마감(16:00 ET) 후 10분 버퍼 |
| 15:40 | 한국 주가 + 경제지표 + 뉴스 | 한국 장마감(15:30 KST) 후 10분 버퍼 |
...
## 데이터 소스 정책 보강 (2026-02-25)
- 정책상 버킷 역할 분리:
  - `econ_market`: 운영 수집/모니터링 버킷
  - `econ_market_backfill_2010_2025`: 백필 검증 기준 버킷
- FRED 결측(`NaN`) 값은 현재 적재 스킵 정책으로 운영됩니다.
- 정합성 평가는 `raw`와 `valid(non-NaN)` 두 관점으로 병행 보고합니다.

## 정책 보완 필요
1. FRED 결측 적재 전략(스킵 vs 결측 플래그)을 최종 확정

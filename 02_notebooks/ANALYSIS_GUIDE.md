...
## 분석 가이드 업데이트 (2026-02-25)
- 백필 정확도 분석은 `econ_market_backfill_2010_2025` 버킷 기준으로 수행합니다.
- 운영 추세 분석은 `econ_market` 버킷을 사용하되, 백필 기준과 혼용하지 않습니다.
- FRED 데이터 분석 시 결측(`NaN`) 466건을 제외한 `valid` 기준 집계를 기본으로 사용합니다.
